...
export default class CrossBookingBankPage {
    ...
    constructor(private page: Page) {
     ...
     private Elements = {
        ...
        SourceCode : '//*[@id="BLK_BRN_OUT_CCT_TXN__SOURCE_CODE|input"]',
        NetworkCode : '//*[@id="BLK_BRN_OUT_CCT_TXN__NETWORK_CODE|input"]',
        TransferCurrecy : '//*[@id="BLK_BRN_OUT_CCT_TXN__TXN_CCY|input"]',
        TransferAmount : '//*[@id="BLK_BRN_OUT_CCT_TXN__TXN_AMOUNT|input"]',
        debitAccountNo : '//*[@id="BLK_BRN_OUT_CCT_TXN__DR_AC_NO|input"]',
        debitordetails : '//*[@id="BLK_BRN_OUT_FICT_MSG__DBTR_FINID_BICFI|input"]',
        creditordetails : '//*[@id="BLK_BRN_OUT_FICT_MSG__CDTR_FINID_BICFI|input"]',
        ...
        exit : '//*[@id="BTN_EXIT_IMG_oj180|text"]',
        enterquery : '//*[@id="EnterQuery_oj17|text"]',
        TransactionReferenceNo : '//*[@id="BLK_BRN_OUT_SWIF_CCT_DRIVER__TXN_REF_NO|input"]',
        executequery : '//*[@id="ExecuteQuery_oj18|text"]',
        authorize : '//*[@id="Authorize_oj8|text"]',
        authorizebtn : '//*[@id="BLK_BRN_OUT_SWIF_CCT_DRIVER__BTN_AUTH_oj21|text"]',
        ok : '//*[@id="BTN_OK_oj0|text"]',
        getrefNo : '//*[@id="BLK_BRN_OUT_SWIF_CCT_DRIVER__TXN_REF_NO"]/div[1]/div/div/div',
     }

// Cross Border Banking 
async handleCrossBankFrame() {
    try {
      // Wait for the iframe to appear in the AO
      const frameElementHandle = await this.page.waitForSelector('//iframe[contains(@title, "Cross Border Outbound FI Credit Transfer Input Detailed")]',{ timeout: 30000 });
    const frame = await frameElementHandle.contentFrame();
   return frame;
    } catch (message) {
     console.log("handleCrossBankFrame() failed:", message);
    }
  }
...
async handleInformationMessageFrame() {
  ...
  '//iframe[contains(@title, "Cross Border Outbound FI Credit Transfer Input Detailed")]', { timeout: 30000 }
);
...
async enterdebitor(debitor){
    const frame = await this.handleCrossBankFrame()
    await frame.waitForSelector(this.Elements.debitordetails, { state: 'visible', timeout: 15000 });
       await frame.locator(this.Elements.debitordetails).clear()
       await frame.locator(this.Elements.debitordetails).fill(debitor)
}

async entercreditor(creditor){
    const frame = await this.handleCrossBankFrame()
    await frame.waitForSelector(this.Elements.creditordetails, { state: 'visible', timeout: 15000 });
       await frame.locator(this.Elements.creditordetails).clear()
       await frame.locator(this.Elements.creditordetails).fill(creditor)
}
