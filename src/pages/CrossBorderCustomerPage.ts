...
export default class CrossBorderCustomerPage {
    ...
    constructor(private page: Page) {
     ...
     private Elements = {
        ...
        CreditBICFI : '//*[@id="BLK_BRN_OUT_CCT_MSG__CDTA_FINID_BICFI|input"]',
        ...
        exit : '//*[@id="BTN_EXIT_IMG_oj178|text"]',
...
async handleCustomerFrame() {
    try {
      // Wait for the iframe to appear in the AO
      const frameElementHandle = await this.page.waitForSelector('//iframe[contains(@title, "Cross Border Outbound FI to FI Customer Credit Transfer Input Detailed")]',{ timeout: 30000 });
    const frame = await frameElementHandle.contentFrame();
   return frame;
    } catch (message) {
     console.log("handleCustomerFrame() failed:", message);
    }
  }
// information message
async handleInformationMessageFrame() {
  ...
  '//iframe[contains(@title, "Cross Border Outbound FI to FI Customer Credit Transfer Input Detailed")]', { timeout: 30000 }
);
