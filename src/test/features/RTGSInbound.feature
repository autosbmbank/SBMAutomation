...
@RTGSINBOUNDScreen @SBM
    Scenario Outline: creating RTGS Inbound Transaction
      ...
      And get Transact reference Number RTGSIN
      ...
      And user SignOff the application
             ...
             Then valdiate the home page tite as "- Oracle Financial Services - ENG - Transaction Input"
             When user enters the function name as "PSDIT2BT" and click search button
      And user clicks on Enter Query in RTGSIN
      And enters Transact reference Number RTGSIN
      And user clicks on Execute Query in RTGSIN
      And user clicks on Authorize tab in RTGSIN
      And user clicks on Authorize button in RTGSIN
      Then clicks on OK button in RTGSIN
